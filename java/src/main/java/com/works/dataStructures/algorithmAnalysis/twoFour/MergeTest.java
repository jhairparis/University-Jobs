	...
	public void run() {
		...
		for (int k = min; k <= max; k += step) {
			...
			for (int x = 0; x < arrayAverage; x++) {
				sw.reset();
				sw.start();
				SortingAlgorithms.mergeSort(arrIncr);
				sw.stop();
				time += sw.totalTime();
			}
		...
		for (int k = min; k <= max; k += step) {
			...
			for (int x = 0; x < arrayAverage; x++) {
				sw.reset();
				sw.start();
				SortingAlgorithms.mergeSort(arrDec);
				sw.stop();
				time += sw.totalTime();
			}
		...
		comparison.addLine(valueK, timeDataOrd, "mergeSort Order");
		comparison.addLine(valueK, timeDataNoOrd, "mergeSort No Order");
